...
namespace Sylphe.Utils
{
	...
	/// when N is <see cref="Count"/>, the number of items in
	/// in the queue. <see cref="Top"/> is an O(1) operation.
	/// Most other operations, especially those required by
	/// <see cref="ICollection{T}"/> have O(N) time complexity.
	/// <para/>
	...
	/// the overall operation has O(log N) time complexity.
	/// <para/>
	...
	public abstract class PriorityQueue<T> : ICollection<T>
	{
		...
		/// <summary>
		/// The <see cref="Capacity"/> reported for
		/// a priorty queue with unbounded capacity.
		/// </summary>
		...
		public override string ToString()
		{
			return Capacity == Unbounded
				? $"Count = {Count}, Capacity = unbounded"
